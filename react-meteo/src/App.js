...
const App = () => {
  const { weatherData, loading, setWeatherData, setLoading, setError } =
    UseWeatherData();
  ...
  const getWeatherData = async (city) => {
    try {
      ...
      const weatherResponse = await axios.get(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${apiKey}&units=${unitType}&lang=it`
      );
              ...
              <Row className="mt-3">
                {weatherData.map((weather, index) => (
                  <Col sm={12} md={6} lg={4} className="mt-3" key={index}>
                    <WeatherCard
                      weatherData={weather}
                      unitTypeSymbol={unitTypeSymbol[unitType]}
                    />
                  </Col>
                ))}
              </Row>
